...
namespace Astralum.World
{
  public class WorldComponent_Debug : WorldComponent
  {
    ...
    [TweakValue(TVCategory, 0f, 1f)] public static float ChromaColR = 1f;

    [TweakValue(TVCategory, 0f, 1f)] public static float ChromaColG = 1f;

    [TweakValue(TVCategory, 0f, 1f)] public static float ChromaColB = 1f;

    [TweakValue(TVCategory, 0f, 1f)] public static float CoronaColR = 1f;

    [TweakValue(TVCategory, 0f, 1f)] public static float CoronaColG = 1f;

    [TweakValue(TVCategory, 0f, 1f)] public static float CoronaColB = 1f;

    #endregion

    #region All Other Properties

    [TweakValue(TVCategory, 0f, 5f)] private static float SurfaceNoiseStrength = 0.03f;

    [TweakValue(TVCategory, -50f, 50f)] private static float CoronaRotationSpeed;

    [TweakValue(TVCategory, 0f, 2f)] private static float ChromaticityIntensity = 1f;

    [TweakValue(TVCategory, 0f, 2f)] private static float CoronaIntensity = 1f;

    [TweakValue(TVCategory, 0f, 2f)] private static float OuterCoronaIntensity = 0.25f;

    [TweakValue(TVCategory, 0.5f, 10f)] private static float ChromaticityFalloffPower = 2f;

    [TweakValue(TVCategory, 0f, 10f)] private static float CoronaPower = 5f;

    [TweakValue(TVCategory, 0f, 10f)] private static float OuterCoronaPower = 6f;

    [TweakValue(TVCategory, 0f, 0.5f)] private static float VariabilityAmount;

    [TweakValue(TVCategory, 0f, 5f)] private static float VariabilitySpeed;
