...
{
  public class WorldComponent_BlackHoleHover : WorldComponent
  {
    private const float DistanceToBlackHoles = 20f;
    private const float HoverRadiusMultiplier = 0.25f;
    ...
    public WorldComponent_BlackHoleHover(RimWorld.Planet.World world) : base(world)
    {
    }
    ...
    public override void WorldComponentOnGUI()
    {
      ...
      if (BlackHoleInteractionRegistry.HoverBlackHoles.Count == 0)
        return;
      ...
      BlackHoleInteractionRegistry.HoverBlackHole? hovered = null;
      float bestDist = float.MaxValue;
      
      foreach (BlackHoleInteractionRegistry.HoverBlackHole blackHole in BlackHoleInteractionRegistry.HoverBlackHoles)
      {
        float hoverRadius = Mathf.Max(MinHoverRadius, blackHole.size * HoverRadiusMultiplier);
        float dist = Vector3.Distance(localHit, blackHole.localSkyPos);
      ...
      DrawTooltip(BlackHoleHoverInfoLineCache.GetLines(hovered.Value));
    }
    
    private static void DrawTooltip(List<BlackHoleHoverInfoLine> lines)
    {
